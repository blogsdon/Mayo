...
#cerebellum
covariatesObj <- synGet('syn5605676')
residualExpressionObj <- synGet('syn5605688')
...
mayoExpression2[dplyr::filter(mayoCovariates,Dxn==1)$ID,] %>%
  ...
  scale %>%
  write.csv(file='mayoADarrayExpressionTCX.csv',quote=F)

#MCI
mayoExpression2[dplyr::filter(mayoCovariates,Dxn==0)$ID,] %>%
  apply(2,utilityFunctions::winsorize) %>%
  scale %>%
  write.csv(file='mayononADarrayExpressionTCX.csv',quote=F)


mayoExpression2 <- apply(mayoExpression2,2,utilityFunctions::winsorize)
mayoExpression2 <- scale(mayoExpression2)

write.csv(mayoExpression2,file='mayoarrayExpressionTCX.csv',quote=F)